...
# PUBLIC TASK HELPERS
def get_links_to_process(**context) -> Dict[str, str]:
    ...
    file_path = _write_links_to_file(links)
    return {"links_file": file_path}


def process_telegraph_link_sync(**context) -> Dict[str, Any]:
    ti = context["ti"]
    file_path: Optional[str] = ti.xcom_pull(
        task_ids="get_telegraph_links"
    ).get("links_file")
